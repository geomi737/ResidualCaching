...
def evaluate(model, data, length, samples, seed=777):
    ...
    for parity, size in [('even', length), ('odd', length - 1)]:
        ...
        for chunk in np.array_split(starts, max(1, math.ceil(len(starts) / 4))):
            ...
            common = logits[:, indices] if model.ratio == 1 else logits
            labels = y[:, indices]
...
def run(args):
    ...
    try:
        ...
        optimizer = torch.optim.AdamW(model.parameters(), lr=args.lr, weight_decay=.01, foreach=False)
        ...
        for step in range(1, args.steps + 1):
            ...
            lr = args.lr * min(1., step / 10) * (.1 + .9 * .5 * (1 + math.cos(math.pi * step / args.steps)))
            ...
            event = {'step': step, 'train_loss': mean_loss, 'lr': lr,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--merge-layer', type=int, default=15)
    ...
    parser.add_argument('--output', default='out-smollm')
