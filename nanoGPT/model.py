...
def boundary_target_indices(length, merge_ratio, device=None):
    """Indices in shifted targets Y for completed windows and singleton tails."""
    full_length = (length // merge_ratio) * merge_ratio
    if full_length == 0:
        return torch.arange(length, device=device)
    return torch.cat((
        torch.arange(merge_ratio - 1, full_length, merge_ratio, device=device),
        torch.arange(full_length, length, device=device),
    ))
...
class WindowTokenMerger(nn.Module):
    ...
    def __init__(self, d_model, merge_ratio=2):
        super().__init__()
        if not isinstance(merge_ratio, int) or merge_ratio < 1:
...
class GPTConfig:
    ...
    def __post_init__(self):
        ...
        if self.merge_ratio > 1 and (not isinstance(self.merge_layer, int)
                                     or not 0 <= self.merge_layer < self.n_layer):
            raise ValueError("merge_layer must be in [0, n_layer) when merging is enabled")
...
class GPT(nn.Module):
    ...
    def __init__(self, config):
        ...
        if config.merge_ratio > 1:
            self.merger = WindowTokenMerger(config.n_embd, config.merge_ratio)
    ...
    def forward(self, idx, targets=None, merge_tokens=True):
        ...
        if targets is not None:
            ...
            ratio = self.config.merge_ratio if merge_tokens else 1
            indices = boundary_target_indices(t, ratio, device=device)
            selected_targets = targets.index_select(1, indices)
            loss = F.cross_entropy(logits.reshape(-1, logits.size(-1)),
                                   selected_targets.reshape(-1), ignore_index=-1)
        else:
            # Generation consumes only the final prediction. Avoid a B*T*V allocation.
