...
def get_batch(split):
    ...
    # Cover every remainder, including R>2; baseline keeps its fixed length.
    ratio = model_args['merge_ratio']
    low = max(1, block_size - ratio + 1)
    seq_len = int(torch.randint(low, block_size + 1, ())) if ratio > 1 else block_size
...
model_args = dict(n_layer=n_layer, n_head=n_head, n_embd=n_embd, block_size=block_size,
                  ...
                  merge_ratio=merge_ratio, merge_layer=merge_layer,
                  use_residual_cache=use_residual_cache, residual_tail=residual_tail)
if init_from == 'scratch':
    ...
    for k, default in [('merge_ratio', 1), ('merge_layer', 2),
                       ('use_residual_cache', True), ('residual_tail', False)]:
    ...
    if merge_ratio > 1:
        raise ValueError('Merged GPT currently supports scratch/resume; pretrained GPT-2 import is baseline-only')
...
model.to(device)
config.update({k: model_args[k] for k in ('merge_ratio', 'merge_layer', 'use_residual_cache', 'residual_tail')})
...
if ddp:
    model = DDP(model, device_ids=[ddp_local_rank], find_unused_parameters=(model_args['merge_ratio'] > 1))
...
while True:
    ...
    if iter_num % log_interval == 0 and master_process:
        ...
        if local_iter_num >= 5: # let the training loop settle a bit
            if model_args['merge_ratio'] == 1:
                mfu = raw_model.estimate_mfu(batch_size * gradient_accumulation_steps, dt)
                running_mfu = mfu if running_mfu == -1.0 else 0.9*running_mfu + 0.1*mfu
        mfu_text = f'{running_mfu*100:.2f}%' if model_args['merge_ratio'] == 1 else 'n/a (compressed context)'
        print(f"iter {iter_num}: loss {lossf:.4f}, time {dt*1000:.2f}ms, mfu {mfu_text}")
    iter_num += 1
    local_iter_num += 1
...
if master_process and not eval_only:
    save_checkpoint() # retain the final update, even between evaluation intervals
